...
export default class Utils {
  ...
  static async createHeadlessAPI(apiver, apivermin, appname, apirescback) {
    ...
    client.setApproveContractInfoHandler(info => {
      let err =
        'Unexpected wasm wallet client transaction in headless wallet: ' + info;
      throw err;
    });
  ...
  static async invokeContractAsyncAndMakeTx(args, bytes) {
    ...
    try {
      let { res } = await Utils.callApiAsync('process_invoke_data', {
  ...
  static async initialize(params, initcback) {
    ...
    try {
      if (Utils.isDesktop()) {
        BEAM = await Utils.createDesktopAPI((...args) =>
          Utils.handleApiResult(...args),
        );
      }
      ...
      if (Utils.isMobile()) {
        try {
          BEAM = await Utils.createMobileAPI((...args) =>
            Utils.handleApiResult(...args),
          );
        } catch (e) {
      ...
      if (!BEAM) {
        return await Utils.initcbackWrapper(initcback, [
          'Failed to create BEAM API',
        ]);
      }
  ...
  static async bulkShaderDownload(shadersData, cback) {
    ...
    const sortedShadersData = shadersData.sort((a, b) =>
  ...
  static async fetchShaderBytes(shaderData) {
    return await fetch(shaderData.path)
      .then(response => response.arrayBuffer())
      .then(bytes => shaderData.prepareAndSetShaderBytes(bytes));
  }
  ...
  static handleString(next) {
    ...
    const afterDot =
      next.indexOf('.') > 0 ? next.substring(next.indexOf('.') + 1) : '0';
    if (
  ...
  static getRateStr(value, rate) {
    ...
    return rate > 0 && value > 0
      ? rateVal > 0.1
        ? Utils.numberWithCommas(rateVal) + ' USD'
        : '< 1 cent'
      : '0 USD';
  }
