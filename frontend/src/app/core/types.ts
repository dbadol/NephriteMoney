export type Pallete = 'green' | 'ghost' | 'purple' | 'blue' | 'red' | 'white' | 'vote-red' | 'gradient' | 'transparent';

export type ButtonVariant = 'regular' | 'ghost' | 'ghostBordered' | 'block' | 'link' | 'icon' | 'custom';

interface Quorum {
    type: string;
    value: number;
}
export interface NephriteAppParams {
    aid: number;
    epoch_dh: number;
    is_admin: number;
